...
def generate_field_inp_2sp_file(
    ...
    electrode_profile=[0, 72.0, 72.0, 72.0, -72.0, 72.0, 72.0, 72.0, 72.0, 72.0, 72.0, 72.0, 72.0, 0]
):
    """
...
def compile_n2dec(exe_dir, bfield):
    ...
    if bfield == 3:
        cpp_file = os.path.join(exe_dir, "pbarcoolpot_fft_wn_2spec_nonpert_Bex3_lxp.cpp")
    else:
        cpp_file = os.path.join(exe_dir, "pbarcoolpot_fft_wn_2spec_nonpert_Bex3_lxp.cpp")
    ...
    # Compiling command: g++ -O2 <cpp_file> -o n2dec
    # compile_command = ["g++", "-O2", "-Wall", cpp_file, "-o", exe_path]
    compile_command = ["g++", "-O2", cpp_file, "-o", exe_path]

    # Compile by submitting external process
...
def run_n2dec(run_number, temperature, num_pos, num_be, density, exe_dir, out_dir):
    ...
    print(f"Running N2DEC at T: {temperature:.2f} K, Num_pos: {num_pos:.2e}, Num_be: {num_be:.2e}, Density: {density:.2e}")
    ...
    stdout_file = os.path.join(out_dir, f"n2dec_stdout_{run_number}_T{temp_copy:.2f}_P{num_pos:.2e}_B{num_be:.2e}_D{density:.2e}.txt")
    ...
    print(f'stdout & stderr logged to n2dec_stdout_{run_number}_T{temp_copy:.2f}_P{num_pos:.2e}_B{num_be:.2e}_D{density:.2e}.txt')
...
def run_and_fit_param(run_number, temp, num_positrons, num_be_ions, density, input_config, exe_dir, res_dir, mcp_path, fit):
    ...
    run_id = f"{run_number}_T{temp:.2f}_P{num_positrons:.2e}_B{num_be_ions:.2e}_D{density:.2e}"
    ...
    run_n2dec(run_number=run_number, temperature=temp, num_pos=num_positrons, num_be=num_be_ions, density=density, exe_dir=exe_dir, out_dir=out_dir)
    chi2 = fit(out_dir=out_dir, mcp_path=mcp_path)
    return (temp, num_positrons, num_be_ions, density, chi2)
...
def calculate_error(run_number, deltas, best_temp, best_num_pos, best_num_be_ions, best_density, best_chi2, input_config, exe_dir, res_dir, mcp_path, fit):
    ...
    for i in range(4):
        ...
        if i == 0:
            run_id_temp1 = f"{run_number}_T{plus[i]:.2f}_P{best_num_pos:.2e}_B{best_num_be_ions:.2e}_D{best_density:.2e}"
            run_id_temp2 = f"{run_number}_T{best_temp:.2f}_P{best_num_pos:.2e}_B{best_num_be_ions:.2e}_D{best_density:.2e}"
            run_id_temp3 = f"{run_number}_T{minus[i]:.2f}_P{best_num_pos:.2e}_B{best_num_be_ions:.2e}_D{best_density:.2e}"
            values_temp = [plus[i], best_temp, minus[i]]
        elif i == 1:
            run_id_pos1 = f"{run_number}_T{best_temp:.2f}_P{plus[i]:.2e}_B{best_num_be_ions:.2e}_D{best_density:.2e}"
            run_id_pos2 = f"{run_number}_T{best_temp:.2f}_P{best_num_pos:.2e}_B{best_num_be_ions:.2e}_D{best_density:.2e}"
            run_id_pos3 = f"{run_number}_T{best_temp:.2f}_P{minus[i]:.2e}_B{best_num_be_ions:.2e}_D{best_density:.2e}"
            values_pos = [plus[i], best_num_pos, minus[i]]
        elif i == 2:
            run_id_be1 = f"{run_number}_T{best_temp:.2f}_P{best_num_pos:.2e}_B{plus[i]:.2e}_D{best_density:.2e}"
            run_id_be2 = f"{run_number}_T{best_temp:.2f}_P{best_num_pos:.2e}_B{best_num_be_ions:.2e}_D{best_density:.2e}"
            run_id_be3 = f"{run_number}_T{best_temp:.2f}_P{best_num_pos:.2e}_B{minus[i]:.2e}_D{best_density:.2e}"
            values_be = [plus[i], best_num_be_ions, minus[i]]
        else:
            run_id_den1 = f"{run_number}_T{best_temp:.2f}_P{best_num_pos:.2e}_B{best_num_be_ions:.2e}_D{plus[i]:.2e}"
            run_id_den2 = f"{run_number}_T{best_temp:.2f}_P{best_num_pos:.2e}_B{best_num_be_ions:.2e}_D{best_density:.2e}"
            run_id_den3 = f"{run_number}_T{best_temp:.2f}_P{best_num_pos:.2e}_B{best_num_be_ions:.2e}_D{minus[i]:.2e}"
    ...
    for i in range(4):
        dchi_i = (perturbed_chis[i][0] - perturbed_chis[i][1]) / (2 * deltas[i])
        for j in range(4):
            dchi_j = (perturbed_chis[j][0] - perturbed_chis[j][1]) / (2 * deltas[j])
            C[i, j] = dchi_i * dchi_j
    ...
    plot_radial_profile_multiple(out_dir_temp1, out_dir_temp2, out_dir_temp3, mcp_path, best_dir, "Temp", values_temp)
    plot_radial_profile_multiple(out_dir_pos1, out_dir_pos2, out_dir_pos3, mcp_path, best_dir, "Pos", values_pos)
    plot_radial_profile_multiple(out_dir_be1, out_dir_be2, out_dir_be3, mcp_path, best_dir, "Be", values_be)
    plot_radial_profile_multiple(out_dir_den1, out_dir_den2, out_dir_den3, mcp_path, best_dir, "Den", values_den)
